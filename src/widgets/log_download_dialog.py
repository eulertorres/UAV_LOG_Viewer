...
class LogDownloadDialog(QDialog):
    ...
    def _build_selection_page(self) -> QWidget:
        ...
        serial_panel = QVBoxLayout()
        serial_panel.addWidget(QLabel("Seriais / agendas"))
        ...
        self.serial_list.setGridSize(QSize(210, 200))
        self.serial_list.setIconSize(QSize(140, 140))
        self.serial_list.itemSelectionChanged.connect(self._on_serial_selected)
        serial_panel.addWidget(self.serial_list, 1)
        self.serial_status_label = QLabel("Nenhum serial disponível")
        serial_panel.addWidget(self.serial_status_label)

        right_panel = QVBoxLayout()
        filters_row = QGridLayout()
        filters_row.addWidget(QLabel("Pesquisar"), 0, 0)
        self.search_edit = QLineEdit()
        self.search_edit.setPlaceholderText("Nome do voo ou texto livre...")
        self.search_edit.textChanged.connect(self._apply_filters)
        filters_row.addWidget(self.search_edit, 0, 1, 1, 3)

        filters_row.addWidget(QLabel("Data inicial"), 1, 0)
        self.start_date_edit = QDateEdit()
        self.start_date_edit.setCalendarPopup(True)
        self.start_date_edit.dateChanged.connect(self._apply_filters)
        filters_row.addWidget(self.start_date_edit, 1, 1)

        filters_row.addWidget(QLabel("Data final"), 1, 2)
        self.end_date_edit = QDateEdit()
        self.end_date_edit.setCalendarPopup(True)
        self.end_date_edit.dateChanged.connect(self._apply_filters)
        filters_row.addWidget(self.end_date_edit, 1, 3)
        ...
        filters_row.addLayout(action_layout, 2, 0, 1, 4)

        right_panel.addLayout(filters_row)
        ...
        right_panel.addWidget(self.flight_list, 1)

        self.selection_label = QLabel("Nenhum voo selecionado")
        right_panel.addWidget(self.selection_label)

        main_split.addLayout(serial_panel, 1)
